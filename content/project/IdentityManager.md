...
This overhead cannot be overcame since whenever transferring to a new destination, the network has to retransmit the data since the packet changed(even though the data remains unchanged). 
...
These scenarios generally have more participants and the overhead can grow linearly as the user grows.

However, using NDN may solve duplicated transferring overhead, it's not panacea. The presence of NDN introduced other problems.
...
Certainly no one want to share personal data with untrusted person.
...
These applications, in turn, will use the certificate to sign all the packets it send.
Other entities in this network can then verify the identity of the packet.

When ever there is a new Application, the following logic check should be performed.
...
He can not access personal data too for the same reason. 
With no valid identity, his Interest will never be replied.

### [NDNCERT](https://github.com/named-data/ndncert)

The problem arise when we have to ask, how to get a user certificate.
...
Because most applications, including our data capturing one, runs on cell phones.
...
We did both and Java-Ndncert approach seems to be more successful. 
Our re-written library is thus called [JNdncert](https://github.com/DataCorrupted/jndncert).
You can check our application [here](https://github.com/DataCorrupted/android-identity-manager) with different branches working on different approach.

### Future

This application is not fully finished yet.
Now we can retrieve an user identity whenever it is requested by the user.
